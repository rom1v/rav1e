...
pub fn cdef_sb_padded_frame_copy<T: Pixel>(
  ...
  for p in 0..3 {
    ...
    for y in 0..((sb_size>>ydec) + pad*2) as isize {
      ...
      let out_row = out_slice.row_mut(0, y);
...
pub fn cdef_filter_frame<T: Pixel>(fi: &FrameInvariants<T>, rec: &mut Frame<T>, bc: &mut BlockContext) {
  ...
  for p in 0..3 {
    ...
    for row in 0..padded_px[p][1] {
      ...
      {
        let cdef_row = &mut cdef_slice.row_mut(0, row as isize)[..2];
        // FIXME this is just to make it compile, but it's incorrect if T == u8
      ...
      {
        let cdef_row = &mut cdef_slice.row_mut(rec_w as isize + 2, row as isize)[..padded_px[p][0] - rec_w - 2];
        for x in cdef_row {
      ...
      {
        let cdef_row = &mut cdef_slice.row_mut(2, row as isize)[..rec_w];
