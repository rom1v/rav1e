...
pub fn cdef_filter_frame<T: Pixel>(fi: &FrameInvariants<T>, rec: &mut Frame<T>, blocks: &FrameBlocks) {
  ...
  let blocks_region = blocks.as_region();
  ...
  for fby in 0..fb_height {
    for fbx in 0..fb_width {
      ...
      let cdef_dirs = cdef_analyze_superblock(&cdef_frame, &blocks_region, sbo, sbo, fi.sequence.bit_depth);
      cdef_filter_superblock(fi, &cdef_frame, rec, &blocks_region, sbo, sbo, cdef_index, &cdef_dirs);
    }
  }
}
