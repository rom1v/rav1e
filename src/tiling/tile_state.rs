...
pub mod test {
  ...
  fn test_tile_state_iter_len() {
    ...
    {
      let mut iter = fs.tile_state_iter_mut(128, 128);
    ...
    {
      let mut iter = fs.tile_state_iter_mut(64, 64);
  ...
  #[inline]
  fn rect<'a, T: Pixel>(region: &PlaneRegionMut<'a, T>) -> (isize, isize, usize, usize) {
  ...
  fn test_tile_state_area() {
    ...
    let tile_states = fs.tile_state_iter_mut(64, 64).collect::<Vec<_>>();
  ...
  fn test_tile_state_write() {
    ...
    {
      let mut tile_states =
        fs.tile_state_iter_mut(64, 64).collect::<Vec<_>>();
