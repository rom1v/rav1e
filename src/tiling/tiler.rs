  ...
  fn test_tile_restoration_write() {
...

    {
...

      {
        ...
        units[0][1].filter = RestorationFilter::Wiener {
          coeffs: [[1, 2, 3], [4, 5, 6]],
        };
      }

      {
        ...
        units[1][0].filter = RestorationFilter::Sgrproj {
          set: 42,
          xqd: [10, 20],
        };
      }

      {
        ...
        units[1][1].filter = RestorationFilter::Sgrproj {
          set: 5,
          xqd: [1, 2],
        };
      }
    ...
    assert_eq!(units[0][1].filter, RestorationFilter::Wiener { coeffs: [[1, 2, 3], [4, 5, 6]] });

    let units = &mut fs.restoration.planes[1].units;
    assert_eq!(units[3][2].filter, RestorationFilter::Sgrproj { set: 42, xqd: [10, 20] });

    let units = &mut fs.restoration.planes[2].units;
    assert_eq!(units[3][1].filter, RestorationFilter::Sgrproj { set: 5, xqd: [1, 2] });
  }
