...
fn sgrproj_stripe_rdu(set: u8, xqd: [i8; 2], fi: &FrameInvariants,
  ...
  let mut out_slice = out.mut_slice(&PlaneOffset{x: 0, y: 0});
  let out_data = out_slice.row(stripe_x as isize, stripe_y as isize);
...
fn wiener_stripe_rdu(coeffs: [[i8; 3]; 2], fi: &FrameInvariants,
  ...
  let mut out_slice = out.mut_slice(&PlaneOffset{x: 0, y: 0});
  let out_data = out_slice.row(0, start_yi as isize);
