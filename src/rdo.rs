...
fn compute_tx_distortion<T: Pixel>(
  ...
  let mut distortion = if skip {
    sse_wxh(
      &ts.input.planes[0].region(Area::BlockStartingAt { bo }),
  ...
  if !best.skip {
    ...
    let mut sads = {
      let edge_buf = {
        ...
        get_intra_edges(
          &rec.edged_subregion(Area::BlockStartingAt { bo }, 1, 1),
          tx_size,
          fi.sequence.bit_depth,
          &ts.input.planes[0].cfg,
          None
        )
      };
      intra_mode_set
    ...
    match partition {
      ...
      PARTITION_VERT => {
        ...
        for (&offset, pmv_idx) in partitions.iter().zip(pmv_idxs) {
          let mode_decision =
            rdo_mode_decision(fi, ts, cw, subsize, offset, &pmvs[pmv_idx]);
...
fn rdo_loop_plane_error<T: Pixel>(sbo: SuperBlockOffset, fi: &FrameInvariants<T>,
                                  ts: &TileStateMut<'_, T>, blocks: &BlocksRegion<'_>,
    ...
    for bx in 0..sb_blocks {
      ...
      if bo.x < blocks.cols() && bo.y < blocks.rows() {
        ...
        if !skip {
          ...
          let xdec = in_plane.plane_cfg.xdec;
          let ydec = in_plane.plane_cfg.ydec;
...
pub fn rdo_loop_decision<T: Pixel>(sbo: SuperBlockOffset, fi: &FrameInvariants<T>,
  ...
  if fi.sequence.enable_cdef {
    ...
    for p in 0..3 {
      ...
      for (rec, inp) in rec_region.rows_iter().zip(
        lrf_input.planes[p].region_mut(Area::StartingAt { x: 0, y: 0 }).rows_iter_mut()
      ).take(height) {
  ...
  let cdef_data = cdef_input.as_ref().map(|input| {
    (input, cdef_analyze_superblock(input, &cw.bc.blocks.as_region(), sbo_0, sbo, bd))
  });
  ...
  loop {
      ...
      for cdef_index in 0..(1<<fi.cdef_bits) {
        if cdef_index != prev_best_index {
          ...
          cdef_filter_superblock(fi, &cdef_input, &mut lrf_input,
                                 &cw.bc.blocks.as_region(), sbo_0, sbo, cdef_index as u8, &cdef_dirs);
          for pli in 0..3 {
            match best_lrf[pli] {
              RestorationFilter::None{} => {
                let err = rdo_loop_plane_error(sbo, fi, ts, &cw.bc.blocks.as_region(), &lrf_input, pli);
              ...
              RestorationFilter::Sgrproj{set, xqd} => {
                ...
                let err = rdo_loop_plane_error(sbo, fi, ts, &cw.bc.blocks.as_region(), &lrf_output, pli);
    ...
    if fi.sequence.enable_restoration && lrf_any_uncoded {
      ...
      if let Some((cdef_input, cdef_dirs)) = cdef_data.as_ref() {
        cdef_filter_superblock(fi, &cdef_input, &mut lrf_input,
                               &cw.bc.blocks.as_region(), sbo_0, sbo, best_index as u8, &cdef_dirs);
      }
