...
fn compute_distortion<T: Pixel>(
  ...
  let input_region = ts.input_tile.planes[0].subregion(Area::BlockStartingAt { bo });
  let rec_region = ts.rec.planes[0].subregion(Area::BlockStartingAt { bo });
  ...
  if !luma_only {
    ...
    if w_uv > 0 && h_uv > 0 {
      for p in 1..3 {
        distortion += sse_wxh(
          &ts.input_tile.planes[p].subregion(Area::BlockStartingAt { bo }),
          &ts.rec.planes[p].subregion(Area::BlockStartingAt { bo }),
          w_uv,
          h_uv
        );
      }
...
fn compute_tx_distortion<T: Pixel>(
  ...
  let mut distortion = if skip {
    sse_wxh(
      &ts.input_tile.planes[0].subregion(Area::BlockStartingAt { bo }),
      &ts.rec.planes[0].subregion(Area::BlockStartingAt { bo }),
      w_y,
      h_y
    )
  } else {
...
pub fn rdo_mode_decision<T: Pixel>(
  ...
  for (i, &ref_frames) in ref_frames_set.iter().enumerate() {
    ...
    if fi.frame_type == FrameType::INTER {
      ...
      let b_me = motion_estimation(fi, ts, bsize, bo, ref_frames[0], cmv, pmv);
  ...
  mv_stack: &Vec<CandidateMV>| {
    let (tx_size, mut tx_type) = rdo_tx_size_type(
      fi, ts, cw, bsize, bo, luma_mode, ref_frames, mvs, false,
    );
    ...
    let mut chroma_rdo = |skip: bool| {
      mode_set_chroma.iter().for_each(|&chroma_mode| {
        ...
        encode_block_a(&fi.sequence, ts, cw, wr, bsize, bo, skip);
        let tx_dist =
          encode_block_b(
            fi,
            ts,
  ...
  mode_set.iter().for_each(|&(luma_mode, i)| {
    ...
    let po = mvs[0].target(frame_bo);
    if ts.tile_rect().contains(po) {
  ...
  if !best.skip {
    ...
    let mut sads = {
      let edge_buf = {
        ...
        let po = bo.plane_offset(&rec.plane_cfg);
  ...
  if best.mode_luma.is_intra() && is_chroma_block && bsize.cfl_allowed() {
    ...
    cw.rollback(&cw_checkpoint);
    if let Some(cfl) = rdo_cfl_alpha(ts, bo, bsize, fi.sequence.bit_depth) {
      let wr: &mut dyn Writer = &mut WriterCounter::new();
      let tell = wr.tell_frac();

      encode_block_a(&fi.sequence, ts, cw, wr, bsize, bo, best.skip);
...
pub fn rdo_cfl_alpha<T: Pixel>(
  ts: &mut TileStateMut<'_, T>, bo: BlockOffset, bsize: BlockSize, bit_depth: usize
) -> Option<CFLParams> {
  let PlaneConfig { xdec, ydec, .. } = ts.input.planes[1].cfg;
...
pub fn rdo_tx_type_decision<T: Pixel>(
  ...
  for &tx_type in tx_types {
    ...
    if is_inter {
      motion_compensate(fi, ts, cw, mode, ref_frames, mvs, bsize, bo, true);
    }
    ...
    let tx_dist = if is_inter {
      write_tx_tree(
        fi, ts, cw, wr, mode, bo, bsize, tx_size, tx_type, false, true, rdo_type, true
      )
    }  else {
      write_tx_blocks(
        fi,
...
pub fn get_sub_partitions_with_border_check(
  ...
  if partition == PARTITION_VERT || partition == PARTITION_SPLIT {
    if four_partitions[1].x + hbsw as usize <= mi_width &&
      four_partitions[1].y + hbsh as usize <= mi_height {
        partitions.push(four_partitions[1]); }
  };
  if partition == PARTITION_HORZ || partition == PARTITION_SPLIT {
    if four_partitions[2].x + hbsw as usize <= mi_width &&
      four_partitions[2].y + hbsh as usize <= mi_height {
        partitions.push(four_partitions[2]); }
  };
  if partition == PARTITION_SPLIT {
    if four_partitions[3].x + hbsw as usize <= mi_width &&
      four_partitions[3].y + hbsh as usize <= mi_height {
        partitions.push(four_partitions[3]); }
  };
    ...
    for bx in 0..sb_blocks {
      ...
      if bo.x < blocks.cols() && bo.y < blocks.rows() {
        ...
        if !skip {
          ...
          let in_bo = sbo.block_offset(bx << 1, by << 1);
...
pub fn rdo_loop_decision<T: Pixel>(sbo: SuperBlockOffset, fi: &FrameInvariants<T>,
  ...
  if fi.sequence.enable_cdef {
    cdef_input = Some(cdef_sb_padded_frame_copy(fi, sbo, &const_rec, 2));
  } else {
    for p in 0..3 {
      let po = sbo.plane_offset(&ts.rec.planes[p].plane_cfg);
      let rec_region = ts.rec.planes[p].subregion(Area::StartingAt {
        x: po.x,
        y: po.y,
      });
      let height = lrf_input.planes[p].cfg.height;
      for (rec, inp) in rec_region.rows_iter().zip(
        lrf_input.planes[p].as_region_mut().rows_iter_mut()
      ).take(height) {
        let w = inp.len().min(rec.len());
        inp[..w].copy_from_slice(&rec[..w]);
      }
    }
  }
  ...
  if fi.sequence.enable_restoration {
    for pli in 0..PLANES {
      let ru = ts.restoration.planes[pli].restoration_unit(sbo);
  ...
  let cdef_data = cdef_input.as_ref().map(|input| {
    (input, cdef_analyze_superblock(input, &cw.bc.blocks.as_const(), sbo_0, sbo, bd))
  });
  ...
  loop {
      ...
      for cdef_index in 0..(1<<fi.cdef_bits) {
        if cdef_index != prev_best_index {
          ...
          cdef_filter_superblock(fi, &cdef_input, &mut lrf_input,
                                 &cw.bc.blocks.as_const(), sbo_0, sbo, cdef_index as u8, &cdef_dirs);
          for pli in 0..3 {
            match best_lrf[pli] {
              RestorationFilter::None{} => {
                let err = rdo_loop_plane_error(sbo, fi, ts, &cw.bc.blocks.as_const(), &lrf_input, pli);
                let rate = if fi.sequence.enable_restoration {
                  cw.count_lrf_switchable(w, &ts.restoration, best_lrf[pli], pli)
                } else {
              ...
              RestorationFilter::Sgrproj{set, xqd} => {
                ...
                let err = rdo_loop_plane_error(sbo, fi, ts, &cw.bc.blocks.as_const(), &lrf_output, pli);
                let rate = cw.count_lrf_switchable(w, &ts.restoration, best_lrf[pli], pli);
                cost[pli] = err as f64 + fi.lambda * rate as f64 / ((1<<OD_BITRES) as f64);
                cost_acc += cost[pli];
              }
    ...
    if fi.sequence.enable_restoration && lrf_any_uncoded {
      ...
      if let Some((cdef_input, cdef_dirs)) = cdef_data.as_ref() {
        cdef_filter_superblock(fi, &cdef_input, &mut lrf_input,
                               &cw.bc.blocks.as_const(), sbo_0, sbo, best_index as u8, &cdef_dirs);
      }
      ...
      for pli in 0..3 {
        ...
        if !ru.lock().unwrap().coded {
          for set in 0..16 {
            ...
            let err = rdo_loop_plane_error(sbo, fi, ts, &cw.bc.blocks.as_const(), &lrf_output, pli);
            let rate = cw.count_lrf_switchable(w, &ts.restoration, current_lrf, pli);
