          ...
          if pmvs[0][r].is_none() {
            ...
            if let Some(pmv) = pmvs[0][r] {
              ...
              if let Some(mv1) = pmvs1 {
                save_block_motion(ts, ts.w_in_b, ts.h_in_b, BlockSize::BLOCK_32X32, sbo.block_offset(0, 0), i, mv1);
              }
              if let Some(mv2) = pmvs2 {
                save_block_motion(ts, ts.w_in_b, ts.h_in_b, BlockSize::BLOCK_32X32, sbo.block_offset(8, 0), i, mv2);
              }
              if let Some(mv3) = pmvs3 {
                save_block_motion(ts, ts.w_in_b, ts.h_in_b, BlockSize::BLOCK_32X32, sbo.block_offset(0, 8), i, mv3);
              }
              if let Some(mv4) = pmvs4 {
                save_block_motion(ts, ts.w_in_b, ts.h_in_b, BlockSize::BLOCK_32X32, sbo.block_offset(8, 8), i, mv4);
              }
            }
          }
        }
