  ...
  T: Pixel,
{
  ...
  fn pred_paeth<'a>(
    ...
    for r in 0..Self::H {
      let row = output.row_mut(0, r as isize);
      for c in 0..Self::W {
  ...
  fn pred_cfl_inner<'a>(
    ...
    assert!(output.len() >= output.plane.cfg.stride * (Self::H - 1) + Self::W);
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
    {
    ...
    let avg: i32 = output.row(0, 0)[0].into();

    for r in 0..Self::H {
      let row = output.row_mut(0, r as isize);
      let luma = &ac[r * Self::W..(r + 1) * Self::W];
      for (v, &l) in row[..Self::W].iter_mut().zip(luma[..Self::W].iter()) {
        *v = T::cast_from(
  ...
  fn pred_directional<'a>(
    ...
    if p_angle < 90 {
      for i in 0..Self::H {
        let row = output.row_mut(0, i as isize);
        for j in 0..Self::W {
          let idx = (i + 1) * dx;
